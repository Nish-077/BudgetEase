...
const ViewBudgets = () => {
  ...
  useEffect(() => {
    const fetchBudgets = async () => {
      try {
        ...
        // console.log("Budgets data:", response);
        setBudgets(response);
        if (response.length > 0) {
          setSelectedBudget(response[0]);
        }
      } catch (err) {
  ...
  // Fetch progress when selected budget changes
  useEffect(() => {
    const fetchProgress = async () => {
      if (!selectedBudget?.budgetId) {
        console.log("No budget ID selected");
        return;
      }
      ...
      setProgressError("");
      try {
        console.log("Making progress request for budget ID:", selectedBudget.budgetId);
        // const response = await axiosInstance.get(
        //   `/financial-target/progress/budget/${selectedBudget.budgetId}`
        // );
          ...
          <div className="bg-white rounded-2xl shadow-lg p-6 lg:col-span-1 overflow-y-auto">
            ...
            {loading ? (
              ...
              <div className="text-center py-8 text-gray-500">
                ...
                <button  onClick={ () => navigate("/create-budget") } className="mt-4 text-green-600 hover:text-green-800 font-medium">
